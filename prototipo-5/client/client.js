...
Template.matchnametemp.matchname=function(){
	var matchName = "None";
	if(Matches.findOne({_id: Session.get('match_id')})){
		matchName = Matches.findOne({_id: Session.get('match_id')}).name;
	};
	return matchName;
}
...
Template.roomplayerstemp.players=function(){	
	var matches_document = Matches.findOne({_id : Session.get('match_id')});
	var players_list;
	var players_names = [];
	if(matches_document){
		players_list = matches_document.players;
...
Template.matchestemp.events = {
	...
	'keydown input#match_creator':function(event){
		if(event.which==13){
			if ($("#match_creator").val()!=''){
				...
				var current_match_id = Matches.findOne({name: $("#match_creator").val()})._id;
				...
				Matches.update({_id : Session.get('match_id')}, {$push: {players: Meteor.userId()}, $inc: {num_players :1}});
				$("#match_creator").val('');
			}
		}
	},
	...
	'click a.linkmatch':function(event){
		...
		var no_limit = Matches.findOne({_id : $(this)[0]._id}).num_players < lim;
		var players_array = Matches.findOne({_id : $(this)[0]._id}).players;
...
Template.roomgametemp.events = {
	...
	'click a#exitgame':function(event){
		...
		var players_array = Matches.findOne({_id : quited_match_id}).players;
		players_array = _.reject(players_array, function(player){ return player == quiter_id; });
		Matches.update({_id : quited_match_id}, {$set:{players : players_array},$inc: {num_players: -1}});

		if(Matches.findOne({_id : quited_match_id}).num_players == 0){
			Matches.remove({_id : quited_match_id});
		}
